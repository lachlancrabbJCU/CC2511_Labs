...
/*! \brief Display progess line for PWM symbol
 * \lingroup pico_term
 */
static inline int display_bar(int level, unsigned short colour)
{
  int i;
  float max_level = 255;
  float percentage = (level / max_level) * 67;
  printf("|");
  term_set_color(clrWhite, colour);
